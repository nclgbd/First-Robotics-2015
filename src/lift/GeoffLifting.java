...
/** This is the lift for the practice robot Geoff. Similar function, different values. */
public class GeoffLifting extends Command {
	...
	/**The part that does the actual lifting. */
	public void lift() {
		if(liftLeft.getRawButton(1)) {
			lift_motor1.set(.20);//moves lift down
			lift_motor2.set(.20);//moves lift down
			
		}else if (liftRight.getRawButton(1)) {
			lift_motor1.set(-.75);//moves lift up
			lift_motor2.set(-.75);//moves lift up
			
		}else {
			lift_motor1.set(-.20);//keeps lift from moving down
			lift_motor2.set(-.20);//keeps lift from moving down
